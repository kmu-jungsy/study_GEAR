...
def test_vcache():
	...
	for bits in [2, 4, 8]:
		...
		scale = scale.view(B, nh, T, hd // group_size, 1).expand(B, nh, T, hd // group_size, group_size).contiguous().view_as(v)
		mn = mn.view(B, nh, T, hd // group_size, 1).expand(B, nh, T, hd // group_size, group_size).contiguous().view_as(v)
		dequant_v = v * scale + mn
...
def test_kcache():
	...
	for bits in [2, 4, 8]:
		...
		scale = scale.view(BS, nh, D, T // group_size, 1).expand(BS, nh, D, T // group_size, group_size).contiguous().view(BS, nh, D, T).transpose(2, 3)
		mn = mn.view(BS, nh, D, T // group_size, 1).expand(BS, nh, D, T // group_size, group_size).contiguous().view(BS, nh, D, T).transpose(2, 3)
		dequant_k = k * scale + mn
...
def test_streaming_kvcache():
    ...
    # 양자화하지 않은 데이터를 처리하는 새로운 코드
    print("Processing non-quantized data")
    our_attn_output_nonquant = None
    key_states_full_nonquant = key_states[:, :, -(key_states.shape[-2] % group_size):, :].contiguous()
    value_states_nonquant = value_states[:, :, -(value_states.shape[-2] % group_size):, :].contiguous()
    for i in range(16):
        if our_attn_output_nonquant is None:
            query_states = torch.randn((BS, nh, query_len, D), device='cuda', dtype=torch.float16)
        else:
            query_states = our_attn_output_nonquant
        key_states_new = torch.randn((BS, nh, query_len, D), device='cuda', dtype=torch.float16)
        value_states_new = torch.randn((BS, nh, query_len, D), device='cuda', dtype=torch.float16)

        key_states_full_nonquant = torch.cat([key_states_full_nonquant, key_states_new], dim=2)
        att_qkfull = torch.matmul(query_states, key_states_full_nonquant.transpose(2, 3))
        our_att_weights_nonquant = torch.softmax(att_qkfull / math.sqrt(D), dim=-1)

        value_states_nonquant = torch.cat([value_states_nonquant, value_states_new], dim=2)
        our_attn_output_nonquant = torch.matmul(our_att_weights_nonquant, value_states_nonquant)

        ref_att_weights = torch.matmul(query_states, key_states_full_nonquant.transpose(2, 3)) / math.sqrt(D)
        ref_att_weights = torch.softmax(ref_att_weights, dim=-1)
        ref_att_out = torch.matmul(ref_att_weights, value_states_nonquant)

        att_weight_gap = (ref_att_weights - our_att_weights_nonquant) / ref_att_weights
        print(f'i {i} non-quantized, mean att weight rel arr: {torch.mean(torch.abs(att_weight_gap))}')
        att_out_gap = (ref_att_out - our_attn_output_nonquant) / ref_att_out
        print(f'i {i} non-quantized, mean att out rel arr: {torch.mean(torch.abs(att_out_gap))}')
...
def test_4d_qmatmul():
	...
	print("Processing non-quantized data")
	ref_out_nonquant = torch.matmul(query_state, k.transpose(2, 3))
	gap_nonquant = (ref_out_nonquant - ref_out_nonquant) / ref_out_nonquant
	gap_nonquant = torch.nan_to_num(gap_nonquant)
	err_nonquant = torch.mean(torch.abs(gap_nonquant)).item()
	print(f'non-quantized err: {err_nonquant}')
